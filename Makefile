# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: mpalkov <mpalkov@student.42barcelo>        +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2022/05/12 15:39:56 by mpalkov           #+#    #+#              #
#    Updated: 2022/09/01 12:27:21 by mpalkov          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

NAME		=	libft.a

SRCS		=	ft_isalpha.c ft_isdigit.c ft_isalnum.c ft_isascii.c ft_isprint.c \
				ft_strlen.c ft_memset.c ft_bzero.c ft_memcpy.c ft_memmove.c \
				ft_strlcpy.c ft_strlcat.c ft_toupper.c ft_tolower.c ft_strncmp.c \
				ft_memcmp.c ft_strchr.c ft_strrchr.c ft_strnstr.c ft_atoi.c \
				ft_calloc.c ft_strdup.c \
				ft_substr.c ft_strjoin.c ft_strtrim.c ft_putchar_fd.c \
				ft_putstr_fd.c ft_putendl_fd.c ft_putnbr_fd.c ft_itoa.c \
				ft_split.c ft_strmapi.c ft_striteri.c ft_memchr.c ft_putchar.c

SRCS_BONUS	=	ft_lstnew_bonus.c ft_lstadd_front_bonus.c \
				ft_lstsize_bonus.c ft_lstlast_bonus.c ft_lstadd_back_bonus.c \
				ft_lstdelone_bonus.c ft_lstclear_bonus.c ft_lstiter_bonus.c \
				ft_lstmap_bonus.c

OBJS		=	$(SRCS:%.c=%.o)

DEPS		=	$(SRCS:%.c=%.d)

OBJS_BONUS	=	$(SRCS_BONUS:%.c=%.o)

DEPS_BONUS	=	$(SRCS_BONUS:%.c=%.d)

CC			=	gcc

CFLAGS		=	-Wall -Wextra -Werror -MD

RM			=	rm -f

BONUS		=	.bonus

%.o: %.c
	$(CC) $(CFLAGS) -I. -c $< -o $@

all: $(NAME)

-include *.d
$(NAME):$(OBJS) libft.h
	ar -crs $(NAME) $(OBJS)

bonus: $(BONUS)


$(BONUS): $(NAME) $(OBJS_BONUS) libft.h
	ar -crs $(NAME) $(OBJS_BONUS)
	@touch $@

#This recipe deletes all the objects and depemndencies in the folder
clean:
	$(RM) $(OBJS) $(DEPS) $(OBJS_BONUS) $(DEPS_BONUS)

#This recipe deletes everything created by make
fclean: clean
	$(RM) $(NAME)
	$(RM) $(BONUS)

#This recipe deletes everything created by make and re-creates it again.
re:	fclean all

.PHONY:	fclean, all, clean, re
